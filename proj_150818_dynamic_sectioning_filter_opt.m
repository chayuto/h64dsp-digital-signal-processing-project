...
for i = 1:sectionsCount - 1
    ...
    fftTreshold =  6.4; %fft thesholding
    ...
    for n = 2:fftLength-1
        ...
        if mask(n) == 0 && mask(n+1) ==1
            startFreq = frequencyIndex(n) - fftFrqGap/2;
        end
        
        if mask(n) == 1 && mask(n+1) ==0
            stopFreq = frequencyIndex(n) + fftFrqGap/2;
            if stopFreq > (fs/2)
                stopFreq = (fs/2) - 1;
            end
...
sound([inAudio; cleanAudio ;outAudio  ])
