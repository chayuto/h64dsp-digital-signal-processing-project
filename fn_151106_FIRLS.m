function [outAudioSection,filterDestinationIndex] = fn_151106_FIRLS(inAudio,fs,sectionBoundary,windowArray,order)
...
%         [b,a] = cheby1(order,3,window/(fs/2))
       ...
       % b = fir1(filterOrder,[lowCutoff highCutoff]/(fs/2));
        fVector = [lowCutoff highCutoff]/(fs/2);
        b = firls(filterOrder,[0 ,fVector, 1],[0 1 1 0]);
        a = 1;
