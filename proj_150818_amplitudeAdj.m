...
for i = 1:sections
    ...
    if i>1 && i ~= sections
        if mask(i) == 0 && mask(i-1) == 0 && mask(i+1) == 0
            outAudio(audioIndex) = inAudio(audioIndex) * factor2;
        elseif  mask(i) == 0
            outAudio(audioIndex) = inAudio(audioIndex) * factor1;
        else
            outAudio(audioIndex) = inAudio(audioIndex);
        end
    else
        if mask(i) == 1
            outAudio(audioIndex) = inAudio(audioIndex);
